...
from aion.shared.runtime.context.builder import AionRuntimeContextBuilder
from aion.shared.runtime.context.models import (
    AgentBehavior,
    AgentEnvironment,
    AgentIdentity,
    AionExtensions,
...
from aion.shared.runtime.context.utils import extract_event
...
class TestAionRuntimeContextBuilder:
    ...
    def test_inbox_none_returns_none(self):
        ...
        rc = _make_mock_rc()
        with patch("aion.shared.runtime.context.builder.A2AInbox.from_request_context", return_value=None):
    ...
    def test_with_distribution_no_event_returns_context_with_identity(self):
        ...
        with patch("aion.shared.runtime.context.builder.A2AInbox.from_request_context", return_value=inbox):
            result = AionRuntimeContextBuilder.from_request_context(rc)
        assert isinstance(result, AionRuntimeContext)
        assert result.event is None
    ...
    def test_key_error_in_build_returns_none(self):
        ...
        rc = _make_mock_rc()
        with patch(
            "aion.shared.runtime.context.builder.AionRuntimeContextBuilder._build_without_distribution",
...
class TestBuildFromDistribution:
    ...
    def test_extract_event_failure_sets_event_none(self):
        ...
        inbox = _make_inbox_with_dist(include_event=True)
        with patch("aion.shared.runtime.context.builder.extract_event", side_effect=ValueError("parse error")):
            result = AionRuntimeContextBuilder._build_from_distribution(inbox)
        assert result.event is None
        assert result.identity is not None
    ...
    def test_identity_build_failure_sets_identity_none(self):
        ...
        inbox = _make_inbox_with_dist(include_event=False)
        with patch(
            "aion.shared.runtime.context.builder.AgentIdentity.from_distribution",
